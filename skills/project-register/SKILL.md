...
## --update (обновить identity-блок, ничего не потеряв)

Когда шаблон identity-блока эволюционировал, а проект зарегистрирован давно:
1. Найди в AGENTS.md проекта маркеры `<!-- hub-kit identity block -->` …
   `<!-- /hub-kit identity block -->`.
2. Замени ТОЛЬКО содержимое между маркерами свежим шаблоном (с подстановкой).
   **Всё ниже закрывающего маркера — local-правила и секция «Деплой» — не трогать
   вообще.** Новые стандартные секции (напр. «## Деплой») добавляй, только если их нет.
3. Покажи diff, коммит после подтверждения. Маркеров нет → это не наш блок, работай
   как при первичной регистрации (спроси).
